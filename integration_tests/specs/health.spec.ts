...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([
        ...
        prisonerFinanceAdvancesApi.stubPing(),
        prisonApi.stubPing(),
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-prisoner-finance-ui')
    })
  })
  ...
  test.describe('Some unhealthy', () => {
    ...
    test('Health check status is down', async ({ page }) => {
      ...
      expect(payload.components.prisonerFinanceAdvancesApi.status).toBe('UP')
